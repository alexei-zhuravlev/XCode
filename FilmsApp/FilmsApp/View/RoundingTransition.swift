...
//  FilmsApp
//
//  Created by Алексей Журавлев on 17.11.2022.
//
...
extension RoundingTransition:UIViewControllerAnimatedTransitioning{
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if transitionProfile == .show {
            if let showedView = transitionContext.view (forKey:
                                                            UITransitionContextViewKey.to)
            {
                let viewCenter = showedView.center
                let viewsize = showedView.frame.size
                round = UIView()
                round.frame = roundFrame(withViewCenter: viewCenter, size: viewsize,
                                         startPoint: start)
            ...
            let transitionModeKey = (transitionProfile == .pop) ? UITransitionContextViewKey.to :
            ...
            if let returnableView = transitionContext.view(forKey: transitionModeKey) {
                ...
                UIView.animate (withDuration: time, animations: {
                    ...
                    if self.transitionProfile == .pop { container.insertSubview (returnableView, belowSubview: returnableView)
