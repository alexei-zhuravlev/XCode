        ...
        do{
            ...
            try realm?.write({
                for item in jsonObjects{
                    ...
                    URLService().dataBacdropsRequest(movieID: item.id!){
                        try! realm?.write({
                            let objectForBackdrops = model.filmObjects?.filter("id == \(item.id!)").first
                            for i in storage.arrayOfBackdrops{
                                objectForBackdrops?.screenshots.append(i)
                                realm?.add(objectForBackdrops!, update: .all)
                            }
                        })
                    }
